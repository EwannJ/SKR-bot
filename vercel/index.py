...
Logs : chaque invocation apparaît dans Vercel (Project > Logs / Runtime
Logs) grâce au module `logging` standard, avec un niveau INFO pour le
déroulement normal et ERROR/WARNING pour les échecs — l'équivalent de ce que
tu avais dans les logs du bot avec ngrok (ex: confirmation d'ouverture du
tunnel), mais ici pour chaque connexion traitée.
...
Le fichier s'appelle `index.py` (et pas `callback.py`) car le runtime Python
de Vercel ne détecte automatiquement un point d'entrée que sous des noms
précis : app.py, index.py, server.py, main.py, wsgi.py ou asgi.py (dans le
dossier racine, src/, app/ ou api/). Un fichier nommé différemment n'est pas
...
log = logging.getLogger("skr_bot.vercel_callback")
...
log.info("Fonction callback vAMSYS initialisée (redirect_uri=%s)", REDIRECT_URI)
...
def verify_state_token(token: str, secret: str, max_age_seconds: int) -> dict | None:
    try:
        ...
        log.warning("Jeton state illisible (base64 invalide)")
        return None

    if len(raw) != _PAYLOAD_LEN + _SIGNATURE_LEN:
        log.warning("Jeton state de longueur inattendue (%d octets)", len(raw))
        return None
    ...
    if not hmac.compare_digest(expected_signature, signature):
        log.warning("Signature de jeton state invalide (falsifié, ou VAMSYS_STATE_SECRET désynchronisé bot/Vercel)")
        return None
    ...
    try:
        ...
        log.warning("Jeton state corrompu (décodage impossible)")
        return None

    age = time.time() - created_at
    if age > max_age_seconds:
        log.warning("Jeton state expiré (%.0fs, max %ds) pour discord_user_id=%s", age, max_age_seconds, discord_user_id)
        return None
...
def supabase_upsert_link(record: dict) -> bool:
    ...
    try:
        ...
        log.error("Erreur réseau upsert Supabase : %s", exc)
        return False
    if resp.status_code >= 300:
        log.error("Échec upsert Supabase (%s) : %s", resp.status_code, resp.text)
        return False
    log.info("Upsert Supabase OK pour discord_user_id=%s (skr_id=%s)", record.get("discord_user_id"), record.get("skr_id"))
    return True
...
def discord_get_member(guild_id: str, user_id: str) -> dict | None:
    ...
    try:
        ...
        log.error("Erreur réseau récupération membre Discord : %s", exc)
        return None
    if resp.status_code != 200:
        log.error("Échec récupération membre Discord (%s) : %s", resp.status_code, resp.text)
        return None
    return resp.json()
...
def apply_pilot_to_member(guild_id: str, user_id: str, pilot_data: dict) -> tuple[bool, str]:
    ...
    if server_config is None:
        log.error("Serveur %s non configuré (SERVERS)", guild_id)
    ...
    try:
        ...
        log.error("Erreur réseau édition membre Discord (user_id=%s) : %s", user_id, exc)
        return True, f"Partiel : pseudo/rôles non appliqués (erreur réseau : {exc})"

    if resp.status_code >= 300:
        log.error("Échec édition membre Discord (%s) : %s", resp.status_code, resp.text)
        return True, f"Partiel : pseudo/rôles non appliqués (code {resp.status_code})"

    log.info("Pseudo/rôle appliqués pour user_id=%s (nick=%r, pilot_id=%s)", user_id, new_nick, pilot_id)
    return True, "OK"
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        log.info("Callback vAMSYS reçu (code=%s, error=%s)", "oui" if code else "non", error)

        if error:
            log.warning("Autorisation vAMSYS refusée : %s", error)
            self._respond(400, f"Autorisation refusée ou erreur vAMSYS : {error}. Tu peux fermer cette page.")
            return

        if not code or not state:
            log.warning("Callback incomplet (code ou state manquant)")
        ...
        if payload is None:
            # Le détail de la raison (signature invalide, expiré...) est déjà
            # loggé dans verify_state_token.
            self._respond(400, "Lien invalide ou expiré. Retourne sur Discord et reclique sur le bouton.")
            return
        ...
        log.info("State valide pour discord_user_id=%s, guild_id=%s", discord_user_id, guild_id)
        ...
        try:
            ...
            log.error("Erreur réseau échange de token vAMSYS : %s", exc)
            self._respond(502, f"Erreur réseau, réessaie plus tard.\n\n[DEBUG] {exc}")
            return

        if resp.status_code != 200:
            log.error("Échec de l'échange de token vAMSYS (%s) : %s", resp.status_code, resp.text)
            self._respond(502, "Erreur lors de la connexion à vAMSYS. Réessaie.")
            return

        access_token = resp.json().get("access_token")
        if not access_token:
            log.error("Réponse vAMSYS invalide : pas d'access_token dans %s", resp.text)
            self._respond(502, "Réponse vAMSYS invalide (pas de token).")
            return

        log.info("Échange de token vAMSYS OK pour discord_user_id=%s", discord_user_id)
        ...
        try:
            ...
            log.error("Erreur réseau récupération profil pilote : %s", exc)
            self._respond(502, f"Erreur réseau, réessaie plus tard.\n\n[DEBUG] {exc}")
            return

        if profile_resp.status_code != 200:
            log.error("Échec récupération profil pilote (%s) : %s", profile_resp.status_code, profile_resp.text)
            self._respond(
                502,
        ...
        try:
            ...
            if identity_resp.status_code == 200:
                ...
                log.warning("Échec récupération identité pilote (%s) : %s", identity_resp.status_code, identity_resp.text)
        except requests.RequestException as exc:
            log.warning("Erreur réseau récupération identité pilote : %s", exc)
        ...
        if not db_ok:
            log.error("Échec d'enregistrement Supabase pour discord_user_id=%s (skr_id=%s)", discord_user_id, skr_id)
            self._respond(
                500,
        ...
        if success and message == "OK":
            log.info("Liaison réussie pour discord_user_id=%s (skr_id=%s)", discord_user_id, skr_id)
            self._respond(
                200,
            ...
            log.warning("Liaison partielle pour discord_user_id=%s : %s", discord_user_id, message)
            self._respond(
                200,
            ...
            log.error("Échec de l'application du pseudo/rôle pour discord_user_id=%s : %s", discord_user_id, message)
